...
package com.sparkedhost.pterodactyl4j.client.ws.handle;
...
public class ConsoleOuputHandler extends ClientSocketHandler {
    ...
    public ConsoleOuputHandler(PteroClientImpl client, ClientServer server, WebSocketManager manager) {
        super(client, server, manager);
    }

    @Override
    public void handleInternally(String content) {
        getManager().getEventManager().handle(new ConsoleOutputEvent(getClient(), getServer(), getManager(), content));
    }
}
