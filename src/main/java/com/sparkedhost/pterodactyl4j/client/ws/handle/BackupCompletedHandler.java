...
import com.sparkedhost.pterodactyl4j.client.ws.events.BackupCompletedEvent;
...
public class BackupCompletedHandler extends ClientSocketHandler {
    ...
    public void handleInternally(String content) {
        getManager().getEventManager().handle(new BackupCompletedEvent(getClient(), getServer(), getManager(), new JSONObject(content)));
    }
}
