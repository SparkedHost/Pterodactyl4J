...
import com.sparkedhost.pterodactyl4j.application.managers.ServerStartupManager;
import com.sparkedhost.pterodactyl4j.requests.Route;
import com.sparkedhost.pterodactyl4j.requests.action.AbstractManagerBase;
...
public class ServerStartupManagerImpl extends AbstractManagerBase implements ServerStartupManager {
    ...
    protected RequestBody finalizeData() {
        ...
        JSONObject obj = new JSONObject()
                .put("startup", shouldUpdate(STARTUP_COMMAND) ? startupCommand : server.getContainer().getStartupCommand())
                .put("environment", shouldUpdate(ENVIRONMENT) ? environment : server.getContainer().getEnvironment().entrySet()
                        .stream().collect(EnvironmentValue.collector()))
                .put("egg", shouldUpdate(EGG) ? egg : server.getEggIdLong())
                .put("image", shouldUpdate(IMAGE) ? image : server.getContainer().getImage())
